...
The same keys work on Markdown headings, so a note can be numbered as `1` / `1.1` / `1.1.1` in both styles.
...
- Every level ends with a period: `1.` `1.1.` `1.1.1.` — a bare `1.1` is not recognised.
...
Headings use the same counters without a trailing period: `# 1 Introduction`, `## 1.1 Scope`, `### 1.1.1 Detail`.
...
The test file stubs `require("obsidian")` and `require("@codemirror/*")` so `main.js` can be loaded in plain Node, then runs 81 assertions over the core: numbering, subtree moves, caret placement, code-fence handling, heading counters and round-trips.
